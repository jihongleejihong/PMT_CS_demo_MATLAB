function mat = ptb2gabor(im_size, meanlum, contrast, freq, phase, tilt, sd, split, tilt2)
	...
	mat = meanlum * (1 + (mkgrating(lineardist(im_size, tilt), freq, phase) * 2 - 1) .* mkgaussian(ovaldist(im_size, split, tilt2), sd) * contrast);
end